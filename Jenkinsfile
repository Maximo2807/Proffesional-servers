pipeline {
    ...
    // Define el tag de la imagen con la fecha y hora de la compilación
    environment {
    ...
    stages {
        ...
        stage('Build & Push') {
            steps {
                // Autenticación con ECR usando la credencial 'aws-lab' configurada
                withCredentials([
                    [$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-lab', accessKeyVariable: 'AWS_ACCESS_KEY_ID', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']
                ]) {
                    sh """
        ...
        stage('Deploy') {
            steps {
                script {
                    // 1. Obtener la definición de tarea actual (Task Definition)
                    withCredentials([
                        [$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-lab', accessKeyVariable: 'AWS_ACCESS_KEY_ID', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']
                    ]) {
                        sh """
